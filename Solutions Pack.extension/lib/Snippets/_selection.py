...
def get_selected_rooms(uidoc=uidoc, exitscript = True):
    ...
    try:
        with forms.WarningBar(title='Selecione os ambientes e clique em "Finish"'):
    ...
    if not selected_rooms:
        error_msg = 'Nenhum ambiente foi selecionado.\nPor favor, tente novamente.'
        forms.alert(error_msg, title='A Seleção dos ambientes falhou.', exitscript=exitscript)

    return selected_rooms

def get_selected_ceilings(uidoc=uidoc, exitscript = True):
    """Function to Pick Ceilings.
    Previously selected rooms will be pre-selected."""
    doc       = uidoc.Document
    selection = uidoc.Selection  # type: Selection

    selected_elements = [doc.GetElement(e_id) for e_id in selection.GetElementIds()]
    selected_elements    = [e for e in selected_elements if type(e) == Ceiling]
    ref_rooms         = [Reference(r) for r in selected_elements]
    ref_preselection  = List[Reference](ref_rooms)

    ISF_Ceilings      = ISelectionFilter_Classes([Ceiling,])
    selected_elements = []

    try:
        with forms.WarningBar(title='Selecione os forros e clique em "Finish"'):
            ref_selected_rooms = selection.PickObjects(ObjectType.Element,
                                                       ISF_Ceilings,
                                                       'Select Ceilings',
                                                       ref_preselection)

        selected_elements  = [doc.GetElement(ref) for ref in ref_selected_rooms]
    except:
        pass

    if not selected_elements:
        error_msg = 'Nenhum forro foi selecionado.\nPor favor, tente novamente.'
        forms.alert(error_msg, title='A Seleção dos forros falhou.', exitscript=exitscript)
...
def pick_property_lines(given_uidoc = uidoc):
    """Function to promt user to select a property line in Revit UI."""
    curve_refs = given_uidoc.Selection.PickObjects(ObjectType.Element, CustomISelectionFilter("-2001265"), "Selecione a(s) Property Lines(s)")
    elements =[]
